...
const send_ws = (client : WebSocket, content : any, type : string, action : string = "default") => {
...
const auth_handler = (ws : WebSocket, query : query) => {
...
const logout_handler = (ws : WebSocket, query : query) => {
    const user : user = transform_user(query.content);
    for (let i = 0; i < record.length; i++)
        if (record[i].user.id === user.id) {
            record.splice(i, 1);
...
const message_handler = (ws : WebSocket, query : query) => {
...
const conversation_handler = (ws : WebSocket, query : query) => {

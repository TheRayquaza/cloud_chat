...
// GET /api/user
// required in req.headers : X-id
export const get_my_user = async (req: Request, res: Response): Promise<void> => {
    const id = req.headers["X-id"] as string;
    controller_logger.info("Get my user");

    try {
        let user = await User.findByPk(id, {attributes: {exclude: ['password_hash']}});
        console.trace("hey");
        if (!user) {
            controller_logger.info("Unable to get your user profile");
            send_error(res, 404, "Unable to your user profile");
        } else
            send_result(res, 200, user);
    } catch (err) {
        controller_logger.error(err);
...
// DELETE /api/user/
// required in req.headers : X-id
export const delete_my_user = async (req: Request, res: Response): Promise<void> => {
    const id = req.headers["X-id"] as string;
    controller_logger.info("Delete my user");

    try {
        const user: User | null = await User.findByPk(parseInt(id, 10));
        if (!user) {
            controller_logger.info("Unable to delete user with id " + id);
            send_error(res, 404, "Unable to find user " + id);
        } else {
...
export const get_my_conversations = async (req: Request, res: Response): Promise<void> => {
    const id = req.headers["X-id"];
    controller_logger.info("get user " + id + " conversations");

    try {
        const user: User | null = await User.findByPk(parseInt(id as string, 10));
        if (!user)
            send_error(res, 404, "User not found");
        else {
